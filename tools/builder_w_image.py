...
def collate_fn(batch):
    ...
    lidar2cam = [torch.tensor(np.array(item[2])) for item in batch]
    lidar2cam = torch.stack(lidar2cam, dim=0)
    cam2img = [torch.tensor(np.array(item[3])) for item in batch]
    cam2img = torch.stack(cam2img, dim=0)
    partial_data = [item[4][0].clone().detach() for item in batch]
    gt_data = [item[4][1].clone().detach() for item in batch]
    gt_label = [item[4][2].clone().detach() for item in batch]
    image = [item[4][3].clone().detach() for item in batch]
    ...
    return taxonomy_ids, model_ids, lidar2cam, cam2img, (partial_data, gt_data, gt_label, image)
...
def load_model(base_model, ckpt_path, logger = None):
    ...
    if state_dict.get('model') is not None:
        ...
        base_ckpt = {k.replace("module.", ""): v for k, v in state_dict['base_model'].items()}
    # adapt to resnet pre-trained model
    elif state_dict is not None:
        base_ckpt = {"img_backbone." + k: v for k, v in state_dict.items()}
    else:
