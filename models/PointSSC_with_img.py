...
from models.deform_attn_utils import DeformableCrossAttentionImgWithPoint
...
class SelfAttnBlockApi(nn.Module):
    ...
    def __init__(
        ...
        for block_token in block_tokens:
            assert block_token in ['attn', 'rw_deform', 'deform', 'graph', 'deform_graph', "deform_img"], f'got unexpect block_token {block_token} for Block component'
            if block_token == 'attn':
            ...
            elif block_token == "deform_img":
                self.local_attn = DeformableCrossAttentionImgWithPoint(dim, n_heads=num_heads, n_levels=kwargs["n_levels"], n_points=kwargs["n_points"])
    ...
    def forward(self, input_dict, idx=None):
        x = input_dict["x"]
        pos = input_dict["coor"]
        feature_list = []
        if self.block_length == 2:
            if self.combine_style == 'concat':
                ...
                if self.local_attn is not None:
                    if self.local_attn._get_name() == "DeformableCrossAttentionImgWithPoint":
                        input_dict["x"] = norm_x
                        local_attn_feat = self.local_attn(input_dict, idx=idx)
                    else:
...
class CrossAttnBlockApi(nn.Module):
    ...
    def __init__(
        ...
        for self_attn_block_token in self_attn_block_tokens:
            assert self_attn_block_token in ['attn', 'rw_deform', 'deform', 'graph', 'deform_graph', "deform_img"], f'got unexpect self_attn_block_token {self_attn_block_token} for Block component'
            ...
            elif self_attn_block_token == "deform_img":
                self.local_self_attn = DeformableCrossAttentionImgWithPoint(dim, n_heads=num_heads, n_levels=kwargs["n_levels"], n_points=kwargs["n_points"])
        ...
        for cross_attn_block_token in cross_attn_block_tokens:
            assert cross_attn_block_token in ['attn', 'deform', 'graph', 'deform_graph', "deform_img"], f'got unexpect cross_attn_block_token {cross_attn_block_token} for Block component'
            if cross_attn_block_token == 'attn':
                ...
                self.local_cross_attn = DeformableCrossAttentionImgWithPoint(dim, n_heads=num_heads, n_levels=kwargs["n_levels"], n_points=kwargs["n_points"])
        ...
        if self.self_attn_block_length == 2:
            if self.self_attn_combine_style == 'concat':
                ...
                if self.local_self_attn is not None:
                    if self.local_self_attn._get_name() == "DeformableCrossAttentionImgWithPoint":
                        input_dict["x"] = norm_q
                        local_attn_feat = self.local_self_attn(input_dict, idx=self_attn_idx)
                    else:
        ...
        if self.cross_attn_block_length == 2:
            if self.cross_attn_combine_style == 'concat':
                ...
                if self.local_cross_attn is not None:
                    if self.local_cross_attn._get_name() == "DeformableCrossAttentionImgWithPoint":
                        input_dict["x"] = norm_q
                        local_attn_feat = self.local_cross_attn(input_dict, idx=self_attn_idx)
                    else:
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, embed_dim=256, depth=4, num_heads=4, mlp_ratio=4., qkv_bias=False, init_values=None,
        ...
        block_style_list=['attn-deform'], combine_style='concat', k=10, n_group=2, *args, **kwargs):
        super().__init__()
        self.k = k
        ...
        for i in range(depth):
            self.blocks.append(SelfAttnBlockApi(
                ...
                n_levels=kwargs["n_levels"], n_points=kwargs["n_points"]
            ))
    ...
    def __init__(self, embed_dim=256, depth=4, num_heads=4, mlp_ratio=4., qkv_bias=False, init_values=None,
        ...
        for i in range(depth):
            self.blocks.append(CrossAttnBlockApi(
                ...
                k=k, n_group=n_group, n_levels=kwargs["n_levels"], n_points=kwargs["n_points"]
            ))

    def forward(self, q, v, q_pos, v_pos, denoise_length=None, input_dict=None):
        if denoise_length is None:
        ...
        for _, block in enumerate(self.blocks):
            q = block(q, v, q_pos, v_pos, self_attn_idx=self_attn_idx,
                      cross_attn_idx=cross_attn_idx, denoise_length=denoise_length,
...
class PointTransformerDecoder(nn.Module):
    ...
    def forward(self, q, v, q_pos, v_pos, denoise_length=None, input_dict=None):
        q = self.blocks(q, v, q_pos, v_pos, denoise_length=denoise_length, input_dict=input_dict)
        return q
...
class PCTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.coarse_pos_embed = nn.Sequential(
            nn.Linear(in_chans, 128),
            nn.GELU(),
            nn.Linear(128, encoder_config.embed_dim)
        )
    ...
    def forward(self, input_dict):
        ...
        if self.training:
            ...
            # forward decoder
            tmp_dict["x"] = q
            tmp_dict["coor"] = coarse
            tmp_dict["pe"] = self.coarse_pos_embed(coarse)
            ...
            q = self.decoder(q=q, v=mem, q_pos=coarse, v_pos=coor, input_dict=tmp_dict)
...
class PointSSCWithImage(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        # Resnet
        img_config = config.img_backbone_config
        self.img_backbone = ResNet(
            img_config.depth,
            num_stages = img_config.num_stages,
            out_indices = img_config.out_indices,
            dilations = img_config.dilations,
            strides = img_config.strides,
            norm_cfg = img_config.norm_cfg,
            norm_eval = img_config.norm_eval,
            style = img_config.style,
            # contract_dilation = img_config.contract_dilation,
        )
        ...
        self.fpn = FPN(
            ...
            start_level = fpn_config.start_level,
            add_extra_convs = fpn_config.add_extra_convs,
            num_outs = fpn_config.num_outs,
            relu_before_extra_convs = fpn_config.relu_before_extra_convs,
        )
    ...
    def forward(self, input_dict):
        ...
        img_feature = self.img_backbone(image)
        img_feature = self.fpn(img_feature)
        input_dict["img_feature"] = img_feature
        q, coarse_point_cloud, denoise_length = self.base_model(input_dict) # B M C and B M 3
