...
from extensions.chamfer_dist import ChamferDistanceL1, ChamferDistanceL1AndSeg
...
class MLP_CONV(nn.Module):
    def __init__(self, in_channel, layer_dims, bn=None):
        ...
        for out_channel in layer_dims[:-1]:
            layers.append(nn.Linear(last_channel, out_channel, 1))
            if bn:
                layers.append(nn.BatchNorm1d(out_channel))
            layers.append(nn.LeakyReLU(0.05))
            last_channel = out_channel
        layers.append(nn.Linear(last_channel, layer_dims[-1], 1))
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, pos_embedding):
        ...
        for i, (self_attn, ff) in enumerate(self.layers):
            if self.out_attention:
                if pos_embedding is not None:
                    x_att, self_att = self_attn(x + pos_embedding)
                    x = x_att + x
                else:
                    x_att, self_att = self_attn(x)
                    x = x_att + x
                out_cross_attention_list.append(x)
            else:
                if pos_embedding is not None:
                    x_att = self_attn(x + pos_embedding)
                    x = x_att + x
                else:
                    x_att = self_attn(x)
                    x = x_att + x
...
class PCTransformer(nn.Module):
    def __init__(self, config):
        ...
        query_config = config.spatial_transformer_config
        query_transformer_config = config.spatial_transformer_config.transformer_config
        ...
        # ================ original coarse points generation ================
        ...
        # ---------------- our coarse points generation method -----------------
        self.spatial_att = SpatialTransformer(query_config.embed_dim,
                                        depth=query_transformer_config.depth,
                                        n_heads=query_transformer_config.n_heads,
                                        dim_single_head=query_transformer_config.dim_single_head,
                                        dim_ffn=query_transformer_config.dim_ffn,
                                        dropout=query_transformer_config.dropout,
                                        out_attention=query_transformer_config.out_attn,
                                        n_points=query_transformer_config.n_points,
                                        )
        self.coarse_upsample_factor = query_num // self.center_num[-1]
        self.coarse_pred = nn.Sequential(
            nn.Linear(query_config.embed_dim, 512),
            nn.GELU(),
            nn.Linear(512, self.coarse_upsample_factor * 3)
        )
        self.mlp_query = nn.Sequential(
            nn.Linear(query_config.embed_dim + 3, 1024),
            nn.GELU(),
            nn.Linear(1024, 1024),
            nn.GELU(),
            nn.Linear(1024, decoder_config.embed_dim)
    ...
    def forward(self, xyz):
        ...
        transform_out_dict = self.spatial_att(x, pe)
        x = transform_out_dict["ct_feat"]
        coarse = self.coarse_pred(x).reshape(bs, self.center_num[-1], self.coarse_upsample_factor, 3)
        coarse = coarse.reshape(bs, -1, 3)
        ...
        if self.training:
            ...
            picked_points, fps_idx = misc.fps_with_idx(xyz, 64)
            picked_x = pointnet2_utils.gather_operation(x.transpose(1, 2).contiguous(), fps_idx).transpose(1,2).contiguous()
            ...
            # produce query
            upsample_x = x.unsqueeze(2).repeat(1, 1, self.coarse_upsample_factor, 1).reshape(
                    bs, self.center_num[-1] * self.coarse_upsample_factor, -1)
            denoided_x = torch.cat((upsample_x, picked_x), dim=1)
...
class PointSSC(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.cd_weight = config.get("cd_weight", 1.0)
        ...
        if self.num_points is not None:
            ...
            self.decode_head = Mlp(self.trans_dim, 512, 3)  # rebuild one point
        else:
        ...
        self.reduce_map = Mlp(self.trans_dim + 1027, self.trans_dim, self.trans_dim * self.factor)
        self.conv_seg = nn.Linear(self.trans_dim, self.class_num)
        self.build_loss_func()

    def build_loss_func(self):
        self.loss_func_with_idx = ChamferDistanceL1AndSeg()
        self.loss_func = ChamferDistanceL1()

    def get_loss(self, ret, gt, epoch=1):
        pred_coarse, denoised_coarse, denoised_fine, pred_fine, seg_output = ret
        
        B, _, _ = pred_coarse.shape
        ...
        # denoise loss
        idx = knn_point(self.factor, gt_points, denoised_coarse) # B n k 
        denoised_target = index_points(gt_points, idx) # B n k 3 
        denoised_target = denoised_target.reshape(gt_points.size(0), -1, 3)
        ...
        # recon loss
        loss_coarse = self.loss_func(pred_coarse, gt_points)
        loss_fine, fine_idx = self.loss_func_with_idx(pred_fine, gt_points)
        loss_recon = loss_coarse + loss_fine
        loss_recon *= self.cd_weight

        # semantic segmentation loss
        # threshold filter
        # thres = 0.2
        # dist_mask = dist1 < thres
        gt_label = torch.gather(gt_seg, 1, fine_idx.long())
        seg_output = seg_output[:, :-denoised_coarse.shape[1], ...]
        seg_output = seg_output.reshape(B, -1, self.class_num).transpose(1, 2)
        ce_loss = F.cross_entropy(seg_output, gt_label.long(), ignore_index=18)
        ce_loss *= self.ce_weight

        return loss_denoised, loss_recon, ce_loss
    ...
    def forward(self, xyz):
        ...
        rebuild_feature = self.reduce_map(rebuild_feature) # B M C -> (mlp) B M S*C
        rebuild_feature = rebuild_feature.reshape(B, M, self.factor, -1)  # B M S C
        relative_xyz = self.decode_head(rebuild_feature)   # B M S 3
        rebuild_points = (relative_xyz + coarse_point_cloud.unsqueeze(-2))  # B M S 3

        # segmentation head
        seg_logit = self.conv_seg(rebuild_feature)
        ...
        if self.training:
            ...
            ret = (pred_coarse, denoised_coarse, denoised_fine, pred_fine, seg_logit)
            return ret
            ...
            seg_logit = seg_logit.reshape(B, -1, self.class_num).transpose(1, 2)
            seg_logit = F.softmax(seg_logit, dim=1)
            fine_seg_output = seg_logit.max(1)[1]

            ret = (coarse_point_cloud, rebuild_points, seg_logit, fine_seg_output)
            return ret